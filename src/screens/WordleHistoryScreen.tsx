...
import { gameDb, type WordleGame } from '../db/games';
...
export default function WordleHistoryScreen() {
  const navigation = useNavigation<NativeStackNavigationProp<RootStackParamList>>();
  const [games, setGames] = useState<WordleGame[]>([]);

  useEffect(() => {
    setGames(gameDb.wordle.findAll());
  }, []);

  const handlePlayGame = (gameId: string) => {
    navigation.navigate('WordleGame', { gameId });
  };

  const handleDeleteGame = async (e: any, gameId: string) => {
    e.stopPropagation();
    await confirmDelete(async () => {
      await gameDb.wordle.remove(gameId);
      setGames(gameDb.wordle.findAll());
    });
  };
  ...
  return (
    <View style={styles.container}>
      <View style={styles.header}>
        <Text style={styles.title}>Wordle History</Text>
